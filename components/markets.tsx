...
type MarketRow = AssetState & { bias: "BULLISH" | "BEARISH" | "NEUTRAL"; biasConf: number };
...
export default function Markets() {
  ...
  useEffect(() => {
    const run = () => {
      const assets = Object.values(getFundState().assets);
      setRows(assets.map((a) => ({
        ...a,
        bias: a.changePct > 0.5 ? "BULLISH" : a.changePct < -0.5 ? "BEARISH" : "NEUTRAL",
        biasConf: 60 + Math.abs(a.changePct) * 5,
      })));
    };
    run();
  ...
  return (
    <div className="bg-[#0b0f19] border border-gray-800 rounded-xl p-6">
      <div className="flex justify-between items-center mb-5">
        ...
        <div className="px-3 py-1 rounded-full bg-purple-500/10 text-purple-400 text-sm">Scanner Active</div>
      </div>
      <div className="space-y-2">
        {rows.map((row) => (
          <div key={row.symbol} className="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-gray-800">
            ...
            <div className="text-right mr-6">
              <p className="text-white text-sm">${row.price.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
        ...
        {rows.length === 0 && <p className="text-gray-600 text-sm text-center py-4">Loading...</p>}
      </div>
    </div>
  );
}
