...
export function FundProvider({ children }: { children: ReactNode }) {
  ...
  const addTrade = useCallback((trade: Trade) => {
    ...
    setPositions((prev) => {
      ...
      if (trade.action === "SELL") {
        ...
        return { ...prev, [trade.symbol]: { ...existing!, size: newSize } };
      }
  ...
  const updateMarket = useCallback((tick: MarketTick) => {
    setMarket((prev) => ({ ...prev, [tick.symbol]: tick }));
  }, []);
    ...
    setPositions((prevPositions) => {
      ...
      for (const sym of Object.keys(updated)) {
        setMarket((prevMarket) => {
          ...
          updated[sym] = { ...updated[sym], currentPrice, unrealizedPnL };
      ...
      setEquityHistory((prev) => {
        const next = [...prev, { equity: newEquity, time: new Date().toLocaleTimeString() }].slice(-200);
  ...
  return (
    <FundContext.Provider value={{
      cash: STARTING_EQUITY, equity, pnl, trades, positions, market,
      equityHistory, drawdown, sharpe, tradeCount,
      addTrade, updateMarket, markToMarket,
    }}>
      {children}
